...
public class ProjectController {
	...
	public String login(@RequestParam("id") String id, @RequestParam("password") String password,
	    ...
	    } else if (!String.valueOf(member.getPassword()).equals(password)) {
	...
	public String main(@RequestParam(value = "searchType", required = false) String searchType,
	    ...
	    if (id != null) {
	        ...
	        // 사원 또는 대리인 경우 자신의 글만 가져옴
	        if ("사원".equals(member.getGrade()) || "대리".equals(member.getGrade())) {
	        ...
	        if ((searchType != null && !searchType.isEmpty()) || 
	            ...
	            if ("사원".equals(member.getGrade()) || "대리".equals(member.getGrade())) {
	                ...
	                writtenContent = service.searchContent(searchType, searchKeyword, approvalStatus, startDate, endDate, "", member.getGrade());
	            }
	        }
	...
	@PostMapping("/EPwrite")
	public String write(EPBoardVO epboardVO, EPHistoryVO historyVO, @RequestParam("action") String action,
			HttpServletRequest request) {
		HttpSession session = request.getSession();
	...
	@PostMapping("/approve")
	public String approve(@RequestParam("seq") int seq, EPHistoryVO historyVO, @RequestParam("c_name") String cName,
			HttpServletRequest request) {
		HttpSession session = request.getSession();
		String grade = (String) session.getAttribute("grade");
		String name = (String) session.getAttribute("name");
	...
	public String updateEP(EPBoardVO epboardVO, EPHistoryVO historyVO, @RequestParam("action") String action,
			HttpServletRequest request, Model model) {
		HttpSession session = request.getSession();
		String name = (String) session.getAttribute("name");
		String grade = (String) session.getAttribute("grade");
	...
	public String postDelegateApproval(@RequestParam("delegate") String delegate,
			@RequestParam("substitute") String substitute, @RequestParam("grade") String grade,
			HttpServletRequest request) {
		HttpSession session = request.getSession();
		String name = (String) session.getAttribute("name");
